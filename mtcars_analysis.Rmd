...
This report examines the relationship between a set of variables and miles per gallon (MPG). Looking only at the transmission type, our findings suggest that cars with an automatic transmission have a higher fuel consumption as cars with a manual transmission. However, this effect can no longer be observed when including other variables in our final and superior model. Based on our findings MPG can be best expressed as linear combination of *Weight*, $Weight^{2}$ and *Quarter mile time*. The entire report including the respective code can be found on [GitHub](https://github.com/alex23lemm/Regression-Models-Project).
